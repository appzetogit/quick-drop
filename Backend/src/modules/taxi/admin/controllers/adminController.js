...
/**
 * Public read, used by the rider app when the customer taps "Cancel ride".
 * `stage` is 'searching' (no driver assigned yet) or 'assigned'; omitting it
 * returns every active reason.
 */
export const getRideCancelReasons = asyncHandler(async (req, res) =>
  ok(res, {
    results: await adminService.listRideCancelReasons({
      stage: req.query?.stage,
      audience: req.query?.audience || 'user',
    }),
  }),
);

export const getAdminRideCancelReasons = asyncHandler(async (req, res) =>
  ok(res, {
    results: await adminService.listRideCancelReasons({
      stage: req.query?.stage,
...
export const createRideCancelReason = asyncHandler(async (req, res) =>
  ok(res, await adminService.createRideCancelReason(req.body)),
);
export const updateRideCancelReason = asyncHandler(async (req, res) =>
  ok(res, await adminService.updateRideCancelReason(req.params.id, req.body)),
);
export const deleteRideCancelReason = asyncHandler(async (req, res) => {
  await adminService.deleteRideCancelReason(req.params.id);
  ok(res, { deleted: true });
});
